...
module.exports = async function handler(req, res) {
  ...
  try {
    await db.runTransaction(async (t) => {
      ...
      if (escrow.payerId !== user.uid && !isAdmin) throw new Error('Only the payer or an admin can release escrow.');
      ...
      logTransaction(t, { userId: escrow.receiverId, type: 'escrow_release', amount: escrow.amount, status: 'completed', reference, metadata: { escrowId, payerId: escrow.payerId, listingId: escrow.listingId } });
    });
  } catch (err) {
