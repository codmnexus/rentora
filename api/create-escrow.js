const { db, FieldValue, verifyAuth, cors, genRef, auditLog, logTransaction, checkRateLimit } = require('./_lib/admin');
...
module.exports = async function handler(req, res) {
  ...
  try {
    await db.runTransaction(async (t) => {
      ...
      if (!walletSnap.exists || (walletSnap.data().balance || 0) < amount) throw new Error('Insufficient balance.');
      ...
      t.set(escrowRef, { payerId: user.uid, receiverId, amount, status: 'held', listingId, reference, createdAt: FieldValue.serverTimestamp() });
      logTransaction(t, { userId: user.uid, type: 'escrow_hold', amount: -amount, status: 'completed', reference, metadata: { escrowId, receiverId, listingId } });
    });
  } catch (err) {
