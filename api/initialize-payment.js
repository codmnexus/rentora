const { db, FieldValue, verifyAuth, cors, genRef, auditLog, checkRateLimit, paystackRequest } = require('./_lib/admin');
...
module.exports = async function handler(req, res) {
  ...
  if (!amount || typeof amount !== 'number' || amount < 100) return res.status(400).json({ error: 'Amount must be at least ₦100.' });
  if (!email) return res.status(400).json({ error: 'Email required.' });

  const reference = genRef('PAY');
  await db.collection('payments').doc(reference).set({
    userId: user.uid, provider: 'paystack', reference, amount, status: 'pending', metadata, createdAt: FieldValue.serverTimestamp(),
  });
  ...
  return res.status(200).json({ authorization_url: result.data.authorization_url, access_code: result.data.access_code, reference });
};
